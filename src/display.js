...
const rarity_colors = {
    ...
    interstellar: "rarity_interstellar",
    metaversal: "rarity_metaversal",
    sacred: "rarity_sacred",
    ethereal: "rarity_ethereal",
    imaginary: "rarity_imaginary",
    hyperdimensional: "rarity_hyperdimensional",
    polychromatical: "rarity_polychromaticall",
    infinitesimal: "rarity_infinitesimal",
}
...
function log_loot(loot_list, is_combat=true) {
    ...
    let message = `${is_combat?"掉落 ":"获取 "} "` + loot_list[0]["item"]["name"] + `" x` + format_number(loot_list[0]["count"]) + `<br>`;
    if(loot_list.length > 1) {
        for(let i = 1; i < loot_list.length; i++) {
            message += (`, "` + loot_list[i]["item"]["name"] + `" x` + format_number(loot_list[i]["count"]) + '<br>');
        }
    }
...
function create_trade_buttons() {
    ...
    trade_button_5.innerText = "1K";
    trade_button_5.setAttribute("data-trade_ammount", 1000);
    ...
    trade_button_10.innerText = "1M";
    trade_button_10.setAttribute("data-trade_ammount", 1000000);
    ...
    trade_button_1000.innerText = "1G";
    trade_button_1000.setAttribute("data-trade_ammount", 1000000000);
    ...
    if(item_key) {
        ...
        Object.keys(character.inventory).forEach(inventory_key => {
            ...
            if(!item_divs[inventory_key]) {
                ...
                if(div_count != item_count) {
                    if(item_count > 1) {
                        item_divs[inventory_key].getElementsByClassName("item_count")[0].innerText = ` x${format_number(item_count)}`;
                    } else {
        ...
        for(let i = 0; i < to_buy.items.length; i++) {
            ...
            if(!item_buying_divs[key]) {
                ...
                if(div_count !== item_count) {
                    if(item_count > 1) {
                        item_buying_divs[key].getElementsByClassName("item_count")[0].innerText = ` x${format_number(item_count)}`;
                    } else {
...
function create_inventory_item_div({key, item_count, target, is_equipped, trade_index}) {
    ...
    if(item_count != 1) {
        item_name_div.innerHTML += `<span class="item_count"> x${format_number(item_count)}</span>`;
    } else if(item_count) {
...
function create_location_choices({location, category, add_icons = true, is_combat = false}) {
    ...
    if(category === "talk") {
        ...
        Object.keys(location.activities).forEach(key => {
            ...
            activity_div.innerHTML = `<span style="color:#d8c0ff"><i class="material-icons">fitness_center</i> </span> `;
        ...
        Object.keys(location.activities).forEach(key => {
            ...
            activity_div.innerHTML = `<span style="color:#ffc0d0"><i class="material-icons">search</i>  `;
            activity_div.classList.add("activity_div", "start_activity");
            activity_div.setAttribute("data-activity", key);
            activity_div.setAttribute("onclick", "start_activity(this.getAttribute('data-activity'));");
...
function update_displayed_stats() { //updates displayed stats
    ...
    Object.keys(stats_divs).forEach(function(key){
        if(key === "crit_rate" || key === "crit_multiplier") {
            ...
            if(character.xp.current_level <= 18){
                ...
                stats_divs[key].innerHTML = `${format_number(character.stats.full[key]*100)}%`;
                update_stat_description(key);
            }
        }
    ...
    if(lgrank >= 20) character_rank_div.innerText = `战力量级:` + lgrank.toFixed(2);
...
function update_displayed_time() {
    ...
    {
        save_button.innerHTML = "<span class='rarity_metaversal'><b>导出(奖励)(不可导出，嘻嘻)</span></b>";
    }
    else{
        save_button.innerHTML = "导出(不可用)";
    }
...
function format_money(num) {
    ...
    if(num > 0) {
        ...
        let ccl = Math.floor((num)/1e39);//重宝点
        if(ccl!=0) value = (`<span class="coin coin_moneyDDc">${ccl.toLocaleString('en-US')}∯</span> `);
        if(ccl>1e18) value = (`<span class="coin coin_moneyOoM">${(Math.log10(num)).toFixed(3)}א</span> `);
...
function update_displayed_skill_bar(skill, leveled_up=true) {
    ...
    */
    ...
    skill_bar_divs[skill.category][skill.skill_id].children[0].children[0].children[0].innerHTML = `${skill.name()} : level ${skill.current_level}`;
    //skill_bar_name
    ...
    if(skill.current_xp !== "Max") {
        ...
        skill_bar_divs[skill.category][skill.skill_id].children[0].children[2].children[0].innerHTML = `${format_number(skill.current_xp)}/${format_number(skill.xp_to_next_lvl)}`;
...
function create_new_bestiary_entry(enemy_name) {
    ...
    stat_0_value.innerHTML = `${format_number(enemy.stats.health*enemy.stats.agility)}`;
    ...
    stat_1_value.innerHTML = `${format_number(enemy.stats.attack*enemy.stats.agility)}`;
    ...
    stat_2_value.innerHTML = `${format_number(enemy.stats.defense*enemy.stats.agility)}`;
    ...
    stat_4_value.innerHTML = `${format_number(Math.round(enemy.stats.agility*enemy.stats.agility))}`;
    ...
    stat_5_value.innerHTML = `${format_number(Math.round(enemy.xp_value * enemy.stats.agility ** 2))}`;
